...
function createOneClient(config, app) {
  ...
  assert(config.enableClient,
    `[egg-kafka] 'enableClient: ${config.enableClient}'is required on config`);

  const clientOptions = config[config.enableClient];
  assert(clientOptions,
    `[egg-kafka] 'enableClient: ${config.enableClient}' is not found on config`);

  app.logger.info('[egg-kafka] kafkaClient options: %s', clientOptions);
